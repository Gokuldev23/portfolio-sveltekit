...
<a href="projects/{title}" class=" grid landscape:grid-cols-[300px_1fr] portrait:grid-cols-1 relative overflow-hidden">
    <h1 style="view-transition-name:{title}-name;" class="text-orange-600 my-3 font-semibold text-2xl portrait:block landscape:hidden capitalize">{title}</h1>
    <div class="relative">
    ...
    <div class="px-4">
        <h1 style="view-transition-name:{title}-name;" class="text-orange-600 font-semibold text-xl portrait:hidden  capitalize">{title}</h1>
        <p style="view-transition-name: {title}-description;" class="mt-2 text-gray-600 text-sm line-clamp-4">{description}</p>
        <div class="flex items-center mt-3 bg-blue-500 text-white px-4 rounded-full w-fit text-sm hover:underline">View Project</div>
    </div>
</a>