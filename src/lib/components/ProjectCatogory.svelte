...
<section class="xl:mb-16 md:mb-10 mb-6">
	...
	<div class="{projects.length>3?"scroll-bar-css":"no-scrollbar"} flex w-full gap-4 overflow-x-scroll snap-x snap-mandatory rounded-lg p-2">
		{#each projects as project}
			<div class="max-w-xs min-w-80 snap-start">
				<Project {project} />
			</div>
		{/each}
	</div>
